...
# Различители потоков. Нечётные константы из общеупотребимых наборов для
# целочисленного перемешивания; конкретные значения роли не играют,
...
# Персистентный генератор мастер-сида. Один на всё время жизни: new_master_seed
# зовётся редко, но в цикле (тесты, симулятор) свежий randomize() на каждый
# вызов даёт коллизии — ADR-004 предупреждает против reliance на randomize().
var _master_rng := RandomNumberGenerator.new()


func _ready() -> void:
	_master_rng.randomize()


## Новый мастер-сид. Гарантированно <= MAX_SEED (53 бита: 21 + 32).
func new_master_seed() -> int:
	var high: int = int(_master_rng.randi()) & 0x1FFFFF
	var low: int = int(_master_rng.randi()) & 0xFFFFFFFF
