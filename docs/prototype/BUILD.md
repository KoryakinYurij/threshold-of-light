...
> **Godot-выполнение после T-02 — только Windows.** Запуск Godot, тесты,
> ручная проверка и экспорт выполняются из этой Windows-копии проекта:
> `D:\Code AI\Games\Godot v1`. VPS Godot не имеет и не запускает; VPS используется
> лишь как файловое зеркало для агента (`~/projects/godot/work/` + `godot-sync`).
...
Перед переключением на ветку `прототип-дипсик` сначала выполнить `git status` и
бережно сохранить незакоммиченные изменения текущей ветки, если они есть.
