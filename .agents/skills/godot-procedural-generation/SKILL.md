...
- **NEVER generate seeds exceeding $2^{53}-1$ for JSON save round-trips** — Godot's `RandomNumberGenerator.seed` is `uint64`, but numbers serialized via JSON pass through IEEE 754 `double` (53-bit mantissa). Seeds $> 2^{53}-1$ (`9007199254740991`) lose precision on load, breaking seed reproducibility. Cap master seeds to `2^53 - 1` (`MAX_SEED = 9007199254740991`).
